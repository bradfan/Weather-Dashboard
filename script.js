      ...
      fetch(onecallURL)
        ...
        .then(function (oneCallData) {
          ...
          if (parseInt(oneCallData.value) < 3) {
            ...
            parseInt(oneCallData.value) > 3 &&
            parseInt(oneCallData.value) < 6
          ) {
  ...
  fetch(forecastURL)
    ...
    .then(function (forecastData) {
      ...
      for (var index = 4; index < forecastArray.length; index = index + 8) {
        ...
        day.innerHTML = "Date: " + forecastArray[index].dt_txt.slice(5, 10);
        dayBox.append(day);
        var temp = document.createElement("p");
        temp.innerHTML =
          "Temperature: " + forecastArray[index].main.temp + " F";
        dayBox.append(temp);
        var humid = document.createElement("p");
        humid.innerHTML =
          "Humidity: " + forecastArray[index].main.humidity + " %";
        dayBox.append(humid);
        var image = document.createElement("img");
        image.setAttribute(
          "src",
          `https://openweathermap.org/img/wn/${forecastArray[index].weather[0].icon}@2x.png`
        );
        dayBox.append(image);
